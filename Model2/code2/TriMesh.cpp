...
float TriMesh::getLength() { return length * scale.x; }
float TriMesh::getWidth() { return width * scale.z; }
float TriMesh::getHeight() { return height * scale.y; }

openGLObject& TriMesh::getOpenGLObject() { return object; }
...
// 获得初始化矩阵
// 修改初始化矩阵
glm::mat4& TriMesh::getInitModelMatrix() {
	return initModelMatrix;
}

void TriMesh::setInitModelMatrix(glm::mat4 m) {
	this->initModelMatrix = m;
}
...
void TriMesh::storeFacesPoints()
{
	...
	// ChangeBy
	// 将物体的底面中心移动到原点，并将整个物体放置在x-y平面上方对每个物体都做同样的处理
	float max_x = -FLT_MAX;
	float max_y = -FLT_MAX;
	float max_z = -FLT_MAX;
	float min_x = FLT_MAX;
	...
	glm::vec3 tran_vector((max_x + min_x) / 2, (min_y + max_y) / 2, (min_z + max_z) / 2 );

	for (int i = 0; i < vertex_positions.size(); i++) {
		vertex_positions[i] = vertex_positions[i] - tran_vector;
	}
...
void TriMesh::generateSquareRepeat(glm::vec3 color, float length, float width)
{
	// 创建顶点前要先把那些vector清空
	cleanData();
	...
	// 顶点纹理坐标
	vertex_textures.push_back(glm::vec2(0, 0));
	vertex_textures.push_back(glm::vec2(length, 0));
	vertex_textures.push_back(glm::vec2(length, width));
	vertex_textures.push_back(glm::vec2(0, width));
...
glm::mat4 Light::getShadowProjectionMatrix()
{
	...
	modelMatrix = this->getInitModelMatrix() * modelMatrix;
	...
	return glm::mat4(
		-ly + floor, 0.0, 0.0, 0.0,
		lx, floor, lz, 1.0,
		0.0, 0.0, -ly + floor, 0.0,
		-lx * floor, -ly * floor, -lz * floor, -ly);
}
...
void Light::setFloor(float _floor) { floor = _floor; };
