...
public:
	...
	void setInitModelMatrix(glm::mat4 m);
	...
	// 重复贴纹理
	void generateSquareRepeat(glm::vec3 color, float length, float width);
...
protected:
	...
	// ChangeBy
	float length;					// 对应x轴方向的长度
	float width;					// 对应y轴方向的长度
	float height;					// 对应z轴方向的长度
	glm::mat4 initModelMatrix;		// 初始变换矩阵

	// ChangeBy
	openGLObject object;			// 绑定openGLobject
