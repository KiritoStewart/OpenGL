...
int MeshPainter::getMeshIndex(std::string name) {
    std::map<std::string, int>::iterator it;
    it = meshes_name.find(name);
    if (it != meshes_name.end())
        return it->second;
    else
        return -1;
}
...
void MeshPainter::addMesh( TriMesh* mesh, const std::string &name, const std::string &texture_image, const std::string &vshader, const std::string &fshader ){
    meshes_name[name] = meshes.size();
    meshes.push_back(mesh);

    // 绑定openGL对象，并传递顶点属性的数据
    bindObjectAndData(mesh, mesh->getOpenGLObject(), texture_image, vshader, fshader);

};



void MeshPainter::drawMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera, bool isShadow, glm::mat4 fatherMatrix) {
	...
	// 物体的变换矩阵
    glm::mat4 modelMatrix = mesh->getInitModelMatrix();
    modelMatrix = fatherMatrix * modelMatrix;
...
void MeshPainter::drawSkybox(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera) {
    ...
    // 传递矩阵
    glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);
    glUniformMatrix4fv(object.viewLocation, 1, GL_FALSE, &view[0][0]);
    glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);
    // 将着色器 isShadow 设置为0，表示正常绘制的颜色，如果是1着表示阴影
    glUniform1i(object.shadowLocation, 0);
...
void MeshPainter::drawMeshes(Light *light, Camera* camera){
    ...
    {
        drawMesh(meshes[i], meshes[i]->getOpenGLObject(), light, camera, true);
    }
};
