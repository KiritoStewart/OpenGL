...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera){
	...
	camera->projMatrix = camera->getProjectionMatrix(false);//透视投影
...
void MeshPainter::drawMesh2(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera) {
...
//传矩阵
void MeshPainter::drawMesh3(glm::mat4 modelMatrix, TriMesh* mesh, openGLObject& object, Light* light, Camera* camera) {
...
void MeshPainter::drawMeshes2(Light* light, Camera* camera) {
    for (int i = 0; i < meshes.size(); i++)
    {
        drawMesh2(meshes[i], opengl_objects[i], light, camera);
    }
};
void MeshPainter::drawMeshes3(glm::mat4 modelMatrix, Light* light, Camera* camera)
{
    for (int i = 0; i < meshes.size(); i++)
    {
        drawMesh3(modelMatrix, meshes[i], opengl_objects[i], light, camera);
    }
}
