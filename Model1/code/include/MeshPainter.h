...
public:
	...
	void drawMesh2(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera);
	void drawMesh3(glm::mat4 modelMatrix, TriMesh* mesh, openGLObject& object, Light* light, Camera* camera);

	// 绘制多个物体
    void drawMeshes(Light *light, Camera* camera);
	void drawMeshes2(Light* light, Camera* camera);
	void drawMeshes3(glm::mat4 modelMatrix, Light* light, Camera* camera);
	// 清空数据
    void cleanMeshes();
